  ...
  function FileUploadRoot(props, ref) {
    ...
    return (
      <ChakraFileUpload.Root {...rest}>
        <ChakraFileUpload.HiddenInput ref={ref as never} {...inputProps} />
        {children}
...
export const FileUploadDropzone = forwardRef<HTMLInputElement, FileUploadDropzoneProps>(
  function FileUploadDropzone(props, ref) {
    ...
    return (
      <ChakraFileUpload.Dropzone ref={ref as never} {...rest}>
...
const FileUploadItem = forwardRef<HTMLLIElement, FileUploadItemProps>(
  function FileUploadItem(props, ref) {
    const { file, showSize, clearable } = props;
    return (
      <ChakraFileUpload.Item file={file} ref={ref as never}>
...
export const FileUploadList = forwardRef<HTMLUListElement, FileUploadListProps>(
  function FileUploadList(props, ref) {
    ...
    return (
      <ChakraFileUpload.ItemGroup ref={ref as never} {...rest}>
        {acceptedFiles.map((file) => (
          <FileUploadItem key={file.name} file={file} showSize={showSize ?? false} clearable={clearable ?? false} />
        ))}
      ...
      <ChakraFileUpload.Trigger asChild>
        ...
        >
          <ChakraFileUpload.Context>
            {({ acceptedFiles }) => {
              if (acceptedFiles.length === 1) return <span>{acceptedFiles[0]?.name}</span>;
              if (acceptedFiles.length > 1) return <span>{acceptedFiles.length} files</span>;
