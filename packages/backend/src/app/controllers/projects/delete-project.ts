...
import type { DeleteProjectHandler } from "@backend/modules/projects/application/handlers/delete";
...
export const inputDeleteProjectSchema = type({
  projectId: 'string',
  workspaceId: 'string',
});

type InputDeleteProject = typeof inputDeleteProjectSchema.infer;

export interface OutputDeleteProject {
  success: boolean;
}

export class DeleteProjectController {
  constructor(
    private readonly usecase: DeleteProjectHandler,
  ) {}

  async handler({ input, ctx }: Request<InputDeleteProject, AuthContext>): Promise<OutputDeleteProject> {
